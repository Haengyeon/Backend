// 카드에 두어 줄로 들어가야 하는 소개글을 줄인다.
//
// 관광공사 overview는 140자짜리도 있고 740자짜리도 있는데 화면에는 몇 줄 안 들어간다.
// 글자 수로만 자르면 "...위치한 음식 거리로, 한" 처럼 말이 끊긴다. 그래서 길이는
// 목표로만 두고 실제로는 문장 끝에서 끊는다. 조금 길어지더라도 말이 끝나 보이는 쪽이 낫다.
//
// 원문은 DB에 그대로 두고 내보낼 때만 줄인다. 나중에 상세 화면에서 전문이 필요하면
// 자르지 않은 값을 그대로 실어 주면 된다.
